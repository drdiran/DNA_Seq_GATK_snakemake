...
SAMP = [os.path.basename(sample) for sample in glob.glob('*_R1.fastq.gz')]
SAMPLES = [sample.split('_R1.fastq.gz')[0] for sample in SAMP]
...
# This rule runs bwa alignment on the preprocessed fastq files
rule bwaAlignment:
     input: fwd="{sample}_R1.fastq",
